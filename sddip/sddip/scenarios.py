...
class ScenarioGenerator:
    ...
    def generate_scenario_dataframe(self, n_buses:int, demand_buses:list, max_value_targets:list, 
            max_relative_variation:float = 0.1) -> pd.DataFrame:
        
        reduced_profile = self.reduce_profile(self.h0_profile, self.reduction_factor)

        base_profiles = [self.scale_profile(reduced_profile, max_value) for max_value in max_value_targets]
        ...
        demand_bus_keys, no_demand_bus_keys = self.create_bus_keys(n_buses, demand_buses)

        # Set loads for the first (deterministic) stage
        for ndb in no_demand_bus_keys:
            scenario_data[ndb] = [0]*self.n_total_realizations

        for b in range(len(demand_buses)):
            scenario_data[demand_bus_keys[b]] = [self.get_rdm_variation(base_profiles[b][0], max_relative_variation)]
        ...
        for t in range(1,self.n_stages):
            for n in range(1,self.n_realizations_per_stage+1):
                ...
                for b in range(len(demand_buses)):
                    scenario_data[demand_bus_keys[b]].append(self.get_rdm_variation(base_profiles[b][t]))

        return pd.DataFrame(scenario_data)

    
    def create_bus_keys(self, n_buses:int, demand_buses:list) -> Tuple:
        demand_bus_keys = []
        no_demand_bus_keys = []
        ...
        return (demand_bus_keys, no_demand_bus_keys)


    def get_rdm_variation(self, base_value:float, max_relative_variation:float = 0.1) -> float:
...
class ScenarioSampler:
    ...
    def generate_samples(self, n_samples:int) -> list:
        ...
        for _ in range(n_samples):
            sample = [rdm.randint(0,self.n_realizations_per_stage-1) for _ in range(self.n_stages-1)]
            sample.insert(0,0)
